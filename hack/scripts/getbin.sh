...
if [ ! -f "hack/bin/k3s-linux-amd64" ]; then
	wget -O hack/bin/k3s-linux-amd64 https://pkg.qucheng.com/qucheng/cli/stable/k3s/k3s-linux-amd64
fi
if [ ! -f "hack/bin/k3s-linux-arm64" ]; then
wget -O hack/bin/k3s-linux-arm64 https://pkg.qucheng.com/qucheng/cli/stable/k3s/k3s-linux-arm64
fi
