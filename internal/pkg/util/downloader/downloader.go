...
func downloadBySystem(localPath, url string, dlog log.Logger) error {
	...
	if err != nil {
		...
		if _, err := qcexec.Command(curlbin, curlargs...).CombinedOutput(); err != nil {
			return err
		}
		return nil
	}
	...
	if _, err := qcexec.Command(wgetbin, wgetargs...).CombinedOutput(); err != nil {
		return err
	}
	return nil
}
