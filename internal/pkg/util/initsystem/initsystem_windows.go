...
func (sysd WindowsInitSystem) ServiceStart(service string) error {
	...
	if err != nil {
		return errors.Errorf("could not access service %s: %v", service, err)
	}
	defer s.Close()
	...
	status, err := s.Query()
	if err != nil {
		return errors.Errorf("could not query service %s: %v", service, err)
	}
	...
	for status.State != svc.Stopped {
		...
		status, err = s.Query()
		if err != nil {
			return errors.Errorf("could not retrieve %s service status: %v", service, err)
		}
	}
	...
	if err != nil {
		return errors.Errorf("could not start service %s: %v", service, err)
	}
...
func (sysd WindowsInitSystem) ServiceRestart(service string) error {
	if err := sysd.ServiceStop(service); err != nil {
		return errors.Errorf("couldn't stop service %s: %v", service, err)
	}
	if err := sysd.ServiceStart(service); err != nil {
		return errors.Errorf("couldn't start service %s: %v", service, err)
	}
...
func (sysd WindowsInitSystem) ServiceStop(service string) error {
	...
	if err != nil {
		return errors.Errorf("could not stop service %s: %v", service, err)
	}
...
func GetInitSystem() (InitSystem, error) {
	m, err := mgr.Connect()
	if err != nil {
		return nil, errors.Errorf("no supported init system detected: %v", err)
	}
	defer m.Disconnect()
