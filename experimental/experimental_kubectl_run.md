---
sidebarTitle: experimental kubectl run
sidebarOrder: 1
---

## experimental kubectl run
...
```
qcadmin experimental kubectl run NAME --image=image [--env="key=value"] [--port=port] [--dry-run=server|client] [--overrides=inline-json] [--command] -- [COMMAND] [args...]
```
